    ...
    public class CategoryHandler(AppDbContext context) : ICategoryHandler
    {
        public async Task<Response<Category?>> CreateAsync(CreateCategoryRequest request)
        {
            ...
            try
            {
                ...
                return new Response<Category?>(category, 201, message: "Categoria criada com sucesso");
            }
            catch (Exception ex)
            {
        ...
        public async Task<PagedResponse<List<Category>?>> GetAllAsync(GetAllCategoryRequest request)
        {
            ...
            {
                return new PagedResponse<List<Category>?>(null, 500, "Não foi possivel recuperar a categoria");
            }
        }
        ...
        public async Task<Response<Category?>> GetByIdAsync(GetCategoryByIdRequest request)
        {
            try
            {
                var category = await context
                    .Categories
                    .AsNoTracking() //para simplificar a consulta e ser mais rapido
